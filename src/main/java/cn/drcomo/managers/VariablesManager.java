...
public class VariablesManager {
    ...
    private String performAddOperation(Variable variable, String currentValue, String addValue, OfflinePlayer player) {
        try {
            ...
            // 尝试钳制到边界值
            String clamped = clampValueToLimitations(variable, result, player);
            if (variable.getLimitations() != null && validateConstraints(clamped, variable.getLimitations(), player)) {
                logger.debug("加法结果已钳制为边界值: " + clamped);
                return clamped;
            }
    ...
    private String performRemoveOperation(Variable variable, String currentValue, String removeValue, OfflinePlayer player) {
        try {
            ...
            String clamped = clampValueToLimitations(variable, result, player);
            if (variable.getLimitations() != null && validateConstraints(clamped, variable.getLimitations(), player)) {
                logger.debug("删除结果已钳制为边界值: " + clamped);
                return clamped;
            }
